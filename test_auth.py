...
def test_site_admin():
    ...
    if OPA_URL is not None:
        ...
        assert authx.auth._is_site_admin(request_data=FakeRequest(site_admin=True).getRequest(), opa_url=OPA_URL, admin_secret=OPA_SECRET, site_admin_key=CANDIG_OPA_SITE_ADMIN_KEY)
        assert not authx.auth._is_site_admin(request_data=FakeRequest().getRequest(), opa_url=OPA_URL, admin_secret=OPA_SECRET, site_admin_key=CANDIG_OPA_SITE_ADMIN_KEY)
...
def test_get_opa_datasets():
    ...
    if OPA_URL is not None:
        ...
        try:
            user_datasets = authx.auth.get_readable_datasets(request_data=FakeRequest().getRequest())
        except requests.HTTPError as e:
            # get_readable_datasets should raise an error
            assert True

        # user1 has controlled4 in its datasets
        user_datasets = authx.auth.get_readable_datasets(request_data=FakeRequest().getRequest(), admin_secret=OPA_SECRET)
        print(user_datasets)
        assert "SYNTHETIC-1" in user_datasets

        # user2 has controlled5 in its datasets
        user_datasets = authx.auth.get_readable_datasets(request_data=FakeRequest(site_admin=True).getRequest(), admin_secret=OPA_SECRET)
        print(user_datasets)
...
def test_put_aws_credential():
    ...
    if VAULT_URL is not None:
        ...
        result, status_code = authx.auth.store_aws_credential(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()),endpoint=endpoint, bucket="test_bucket", access="test", secret="secret", vault_url=VAULT_URL, vault_s3_token=VAULT_S3_TOKEN)
        print(result, status_code)
        assert status_code == 200

        # try getting it with a non-site_admin token
        result, status_code = authx.auth.get_aws_credential(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()), vault_url=VAULT_URL, endpoint=endpoint, bucket="test_bucket", vault_s3_token=None)
        print(result)
        assert "errors" in result

        # try getting it with a site_admin token
        result, status_code = authx.auth.get_aws_credential(token=authx.auth.get_auth_token(request_data=FakeRequest(site_admin=True).getRequest()), vault_url=VAULT_URL, endpoint=endpoint, bucket="test_bucket", vault_s3_token=None)
...
def test_get_s3_url():
    ...
    if MINIO_URL is not None:
        if VAULT_URL is not None:
            result, status_code = authx.auth.store_aws_credential(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()),endpoint=MINIO_URL, bucket="test", access=MINIO_ACCESS_KEY, secret=MINIO_SECRET_KEY, vault_url=VAULT_URL, vault_s3_token=VAULT_S3_TOKEN)
            assert result['url'] in MINIO_URL
            minio = authx.auth.get_minio_client(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()), s3_endpoint=MINIO_URL, bucket="test")
            assert minio['endpoint'] == MINIO_URL
        else:
            warnings.warn(UserWarning("VAULT_URL is not set"))
        minio = authx.auth.get_minio_client(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()), s3_endpoint=MINIO_URL, access_key=MINIO_ACCESS_KEY, secret_key=MINIO_SECRET_KEY, bucket="test")
    else:
        minio = authx.auth.get_minio_client(token=authx.auth.get_auth_token(request_data=FakeRequest().getRequest()))
    filename = Path(fp.name).name
    minio['client'].put_object(minio['bucket'], filename, fp, Path(fp.name).stat().st_size)
    fp.close()

    url, status_code = authx.auth.get_s3_url(request_data=FakeRequest().getRequest(), object_id=filename, s3_endpoint=minio['endpoint'], bucket=minio['bucket'], access_key=minio['access'], secret_key=minio['secret'])
    print(url)
    assert status_code == 200
...
def test_get_public_s3_url():
    url, status_code = authx.auth.get_s3_url(request_data=FakeRequest().getRequest(), public=True, bucket="1000genomes", s3_endpoint="http://s3.us-east-1.amazonaws.com", object_id="README.ebi_aspera_info", access_key=None, secret_key=None, region="us-east-1")
    response = requests.get(url)
    print(response.text)
